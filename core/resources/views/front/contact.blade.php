						...
						<div class="contact-info-item text-center">
							...
							@php
								...
								for ($i=0; $i < count($femail); $i++) { 
									echo '<p style="font-size:100%;">'.$femail[$i].'</p>';
								}
							@endphp
						</div>
						<div class="contact-info-item text-center">
							...
							<pstyle="font-size:100%;">{{ $setting->address }}</p>
						</div>
						<div class="contact-info-item text-center">
...
<section class="conatct-section section-gap" id="contact">
	<div class="container">
		...
		<div class="contact-form-area wow fadeInUp" data-wow-delay="0.3s">
			<div class="row">
				...
				<script>
					function setAdvisor(select) {
						let img = $(select).find(':selected').data('img');
						let name = $(select).find(':selected').data('name');

						$("#agent_img").attr('src', "{{ asset('assets/front/img/') }}/" + img);
						$("#agent_name").html(name);
					}
				</script>
				<div class="col-lg-5">
					<div class="contact-thumb mb-md-gap-50 text-center">
						<div class="mb-20">
							<h2>Agent</h2>
						</div>
						<img src="{{ asset('assets/front/img/'.$sinfo->contact_form_image) }}" alt="" id="agent_img">
							...
							<div class="row">
								<div class="col-lg-12">
									<div class="input-group mt-30">
										<select class="text-white" name="contact_service_category_id" onchange="setAdvisor(this)">
											<option>--- Select a Service ---</option>
											@foreach($contact_services as $contact_service)
												<optgroup label="{{ $contact_service->name }}">
													@foreach ($contact_service->contactServicesCategories as $category)
														<option value="{{ $category->id }}" 
															data-img="{{ $category->agent_img }}"
															data-name="{{ $category->agent_name }}"
														>
															{{ $category->name }}
														</option>
													@endforeach
